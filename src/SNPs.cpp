...
void SNPs::MCMC(gsl_rng *r){
	llk();
	vector<double> s_segeffects;
	vector<double> s_snpeffects;
	for (int i = 0; i < nannot; i++) s_snpeffects.push_back(0);
	for (int i = 0; i < nsegannot; i++) s_segeffects.push_back(0);
	int ns = 0;

	string outMCMC = params->outstem+".MCMC";
	ofstream outr(outMCMC.c_str());
	outr << "i ";
	if (!params->finemap) outr << "pi ";
	for (int i = 0; i < nannot; i++) outr<< annotnames[i] << " ";
	for (int i = 0; i < nsegannot; i++) outr<< segannotnames[i] << " ";
	outr "lk\n";
	for (int i = 0; i < params->burnin; i++) {
		...
		if (i % params->sampfreq ==0) {
			...
			for (int i = 0; i < nannot; i++) outr<< lambda[i] << " ";
			for (int i = 0; i < nannot; i++) outr<< seglambda[i] << " ";
			outr << data_llk  <<"\n";
			for (int i = 0; i < nannot; i++) cout<< lambda[i] << " ";
			for (int i = 0; i < nannot; i++) cout<< seglambda[i] << " ";
			cout << data_llk  <<"\n";
		}
	}
	int nsamp = 0;
	...
	for (int i = 0; i < params->nsamp; i++){
		...
		if (i % params->sampfreq ==0) {
			...
			for (int i = 0; i < nannot; i++) {
				outr<< lambda[i] << " ";
				s_snpeffects[i] += lambda[i];
			}
			for (int i = 0; i < nannot; i++){
				s_segeffects[i]+= seglambda[i];
				outr<< seglambda[i] << " ";
			}
			outr << data_llk  <<"\n";
			for (int i = 0; i < nannot; i++) cout<< lambda[i] << " ";
